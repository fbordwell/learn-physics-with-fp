...
unitSphere =
  Surface
    (\(th, phi) -> cart (sin th * cos phi) (sin th * sin phi) (cos th))
    0
    pi
    (const 0)
    (const $ 2 * pi)
...
unitSphere' :: Surface
unitSphere' = Surface (uncurry (sph 1)) 0 pi (const 0) (const $ 2 * pi)

parabolaSurface :: Surface
parabolaSurface = Surface (\(x, y) -> cart x y 0) (-2) 2 (\x -> x * x) (const 4)

shiftSurface :: Vec -> Surface -> Surface
shiftSurface d (Surface g sl su tl tu) = Surface (shiftPosition d . g) sl su tl tu

centeredSphere :: R -> Surface
centeredSphere r = Surface (uncurry (sph r)) 0 pi (const 0) (const $ 2 * pi)

sphere :: R -> Position -> Surface
sphere radius center = shiftSurface (displacement (cart 0 0 0) center) (centeredSphere radius)

northernHemisphere :: Surface
northernHemisphere = Surface (uncurry (sph 1)) 0 (pi / 2) (const 0) (const $ 2 * pi)

disk :: R -> Surface
disk radius = Surface (\(s, phi) -> cyl s phi 0) 0 radius (const 0) (const (2 * pi))

unitCone :: R -> Surface
unitCone theta = Surface (\(r, phi) -> sph r theta phi) 0 1 (const 0) (const (2 * pi))
...
centeredCylinder ::
  ...
  R -> -- height
  Volume -- cylinder
centeredCylinder radius height = Volume cylindrical 0 radius (const 0) (const (2 * pi)) (\_ _ -> 0) (\_ _ -> height)
